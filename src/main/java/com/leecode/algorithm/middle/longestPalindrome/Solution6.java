...
public class Solution6 {
    ...
    public String longestPalindrome(String s) {
        ...
        // maxRadius[i]是cs[i]每个位置的最大回文半径
        // 比如：cs=[#c#a#b#a#c#]，maxRadius=[1,2,1,2,1,6,1,2,1,2,1]
        int[] maxRadius = new int[cs.length];
        ...
        for (int i = 0; i != cs.length; i++) {
            // 第一句代码:每轮循环时,i至少不需要验证的区域,先给到maxRadius[i],解释如下:
            // right<=i:i超过了right，无法优化，不用验证的区域就是maxRadius[i]本事=回文半径为1
            // right>i:i没有超过right，可以优化，至少不需要验证的区域：Math.min(maxRadius[2 * index - i], right - i)
            // 以index为中心i的对称点是(2*index-i)
            maxRadius[i] = right > i ? Math.min(maxRadius[2 * index - i], right - i) : 1;
            // 第二句代码:在i位置尝试往外扩最长回文半径长度maxRadius[i]:
            // 如果扩成功maxRadius[i]++;否则立刻停止扩的过程break
            while (i + maxRadius[i] < cs.length && i - maxRadius[i] >= 0) {
                if (cs[i + maxRadius[i]] == cs[i - maxRadius[i]])
                    maxRadius[i]++;
                else {
                    break;
                }
        ...
        // max记录maxRadius[i]中最大值：maxRadius[i]最大值就能算出原字符串的最长回文子串长度
